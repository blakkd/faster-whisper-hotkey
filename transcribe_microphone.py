...
def get_initial_choice(stdscr):
    """Get initial user choice using curses."""
...
class MicrophoneTranscriber:
    ...
    def transcribe_and_send(self, audio_data):
        """Transcribe audio and send text."""
        try:
...
def main():
    while True:
        ...
        if initial_choice == "Choose New Settings":
            device_name = curses.wrapper(lambda stdscr: curses_menu(stdscr, "", [src.name for src in pulsectl.Pulse().source_list()]))
            model_size = curses.wrapper(lambda stdscr: curses_menu(stdscr, "", accepted_models))
            compute_type = curses.wrapper(lambda stdscr: curses_menu(stdscr, "", accepted_compute_types))
            device = curses.wrapper(lambda stdscr: curses_menu(stdscr, "", accepted_devices))
            language = curses.wrapper(lambda stdscr: curses_menu(stdscr, "", accepted_languages))
