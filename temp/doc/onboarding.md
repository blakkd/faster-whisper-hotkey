...
**faster-whisper-hotkey** (v0.4.9) is a push-to-talk speech-to-text (STT) transcription tool for Linux. The core workflow: **hold a hotkey while speaking, release it, and the transcribed text is automatically pasted into whatever text field is currently focused.**
...
|
+-- tests/                             # 15 test files, comprehensive coverage
...
- **`temp/audio_data/transcription_results.txt`** shows benchmark results for "hey my friend, how are you doing?" across all models and device/precision combos -- all 15 configs pass
